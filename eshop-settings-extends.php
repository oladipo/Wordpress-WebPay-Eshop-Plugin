...
class eshop_setting_merchant extends eshop_metabox_class {
	function on_load_page() {
		...
		add_meta_box('eshop-m-microzahlen', __('microzahlen','eshop'), array(&$this, 'microzahlen_box'), $this->pagehook, 'normal', 'core');		
		add_filter('eshop_error_messages',array(&$this, 'eclass_errors'));
...
function microzahlen_box($eshopoptions) {
		global $wpdb;
		?>
		<fieldset>
			<?php 
			if(isset($eshopoptions['microzahlen']))
				$microzahlen = $eshopoptions['microzahlen'];
			else
				$microzahlen['email']=$microzahlen['APIPassword'] = $microzahlen['APIUsername']= $microzahlen['MerchantRef']='';
			
			$this->show_img('microzahlen');
			?>
			<p class="cbox"><input id="eshop_methodh" name="eshop_method[]" type="checkbox" value="microzahlen"<?php if(in_array('microzahlen',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_methodh" class="eshopmethod"><?php _e('Accept payment by microzahlen','eshop'); ?></label></p>
			<p><em><?php _e('All fields are required.','eshop'); ?></em></p>
			<div class="microzahlenhi">
			<h3><?php _e('The following information must be entered at microzahlen','eshop'); ?></h3>
			<p><?php _e('The following 2 links need to be entered into the Technical information > transaction feedback page.','eshop'); ?></p>
			<p><strong><?php _e('HTTP redirection in the browser/URL of the merchant\'s post-payment page:','eshop'); ?></strong></p>
			<p><?php _e('If the payment\'s status is "accepted", "on hold" or "uncertain".','eshop'); ?><br />
			<code><?php echo add_query_arg('eshopaction','microzahlenipn',get_permalink($eshopoptions['cart_success'])); ?></code></p>
			<p><?php _e('If the payment\'s status is "cancelled by the client" or "too many rejections by the acquirer".','eshop'); ?><br />
			<code><?php echo add_query_arg('eshopaction','cancel',get_permalink($eshopoptions['cart_cancel'])); ?></code></p>
			<p><?php _e('The following link needs to be entered into the Technical information > Data and origin verification page.','eshop'); ?></p>
			<p><strong><?php _e('Checks for e-Commerce:','eshop'); ?></strong></p>
			...
			<label for="eshop_microzahlenMerchantRef"><?php _e('Merchant ID','eshop'); ?></label><input id="eshop_microzahlenMerchantRef" name="microzahlen[MerchantRef]" type="text" value="<?php echo $microzahlen['MerchantRef']; ?>" size="20" /><br />
			<label for="eshop_microzahlenAPIUsername"><?php _e('API Username','eshop'); ?></label><input id="eshop_microzahlenAPIUsername" name="microzahlen[APIUsername]" type="text" value="<?php echo $microzahlen['APIUsername']; ?>" size="40" /><br />
			<label for="eshop_microzahlenAPIPassword"><?php _e('API Password','eshop'); ?></label><input id="eshop_microzahlenAPIPassword" name="microzahlen[APIPassword]" type="text" value="<?php echo $microzahlen['APIPassword']; ?>" size="40" /><br />
			<label for="eshop_microzahlenemail"><?php _e('Email address','eshop'); ?></label><input id="eshop_microzahlenemail" name="microzahlen[email]" type="text" value="<?php echo $microzahlen['email']; ?>" size="30" maxlength="50" /><br />
	...
	function on_save_changes() {
		...
		//microzahlen
     	$microzahlenpost['MerchantRef']=esc_sql($_POST['microzahlen']['MerchantRef']);
		$microzahlenpost['APIUsername']=esc_sql($_POST['microzahlen']['APIUsername']);
		$microzahlenpost['APIPassword']=esc_sql($_POST['microzahlen']['APIPassword']);
		$microzahlenpost['email']=esc_sql($_POST['microzahlen']['email']);
		$eshopoptions['microzahlen']=$microzahlenpost;
