...
if (!function_exists('eshop_create_dirs')) {
	function eshop_create_dirs(){
		...
		if(wp_mkdir_p( $upload_dir )){
			...
			if(wp_mkdir_p( $eshop_goto )){
				$files=array('paypal','payson','cash','epn','webtopay','authorizenet', 'ideallite','ogone','bank','microzahlen');
...
if(isset($eshopoptions['version'])){
	...
	*** Enter your name, title, and email here ***
	...
	$wpdb->query("INSERT INTO ".$table." (id,emailType,emailSubject,emailContent) VALUES ('10','".__('Automatic microzahlen email','eshop')."','$esubject','')"); 	
